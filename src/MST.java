...
class MSTworld {
	...
	private static int curr_threads = 0;

    // Constructor
    //
    public MSTworld(int N, long SD, Coordinator C, int num) {
        n = N;
        sd = SD;
        coord = C;
        numThreads = num;
    ...
    //
    class TriangulateAction extends RecursiveAction {
        private int l;
        private int r;
        private int low0;
        private int high0;
        ...
        public TriangulateAction(int l, int r, int low0, int high0,
        ...
        protected void compute(){
            ...
            try {
                ...
                if (i < l) {
                    // empty left half
                    TriangulateAction left_half = new TriangulateAction(j, r, low1, high1, mid, high0, 1-parity);
                    left_half.fork();
                    left_half.join();
                } else if (j > r) {
                    // empty right half
                    TriangulateAction right_half = new TriangulateAction(l, i, low1, high1, low0, mid, 1-parity);
                    right_half.fork();
                    right_half.join();
                } else {
                    // divide and conquer
...
//                    invokeAll(left_half, right_half);
	...
	private static volatile int index = 0; // keep track of partitions
    private static volatile boolean solved = true; // make sure while loop exits when finished
	
	class KruskalParallel extends Thread {
        SortedSet<edge> edges;
        int i;
        int thread;
        
        public KruskalParallel(SortedSet<edge> edges, int i, int thread){
            this.edges = edges;
            this.i = i;
            this.thread = thread;
        }
        ...
        public void run(){
            ...
            if(n%thread == 0){
                ...
                start = i*(n-n%thread)/thread;
            }
                        
            while(start > index && solved){   
                for(edge e: edges){             
                    if(!e.isCycle){
                        point st1 = e.points[0].subtree();
                        point st2 = e.points[1].subtree();
    ...
    public void KruskalSolve() throws Coordinator.KilledException {
        ...
        int p_start = 0; 
        boolean fromSrc = true;
        boolean toDest = false;
        int p = 0;
        int thread_id = 0;
        int interval = 0;
        
        KruskalParallel[] KruskalThreads = new KruskalParallel[curr_threads];
        if(nEdges%(curr_threads + 1) == 0){
            interval = nEdges/(curr_threads + 1);
        }
        else{
            interval = (nEdges - nEdges%(curr_threads + 1))/(curr_threads + 1);
        }
        ...
        for (edge e : edges) {   
            if(p_start%interval == 0 && fromSrc == true){
                src = e;                
                toDest = true;
                fromSrc = false;
                p = p_start;
            }       
            if(p_start == p+interval && toDest == true){
                dest = e;
                fromSrc = true;
                toDest = false;
            
                KruskalThreads[thread_id] = new KruskalParallel(edges.subSet(src, dest), thread_id, curr_threads+1);
                KruskalThreads[thread_id].start();
                thread_id++;
                p_start--;
            }
    ...
    //
    public void DwyerSolve() throws InterruptedException {
        ForkJoinPool pool = new ForkJoinPool(numThreads);
        pool.execute(new TriangulateAction(0, n-1, minx, maxx, miny, maxy, 0));
        pool.shutdown();
        while (!pool.awaitTermination(1, TimeUnit.SECONDS)) {}

        System.out.println("Dwyer pool is closed");
    }
}
